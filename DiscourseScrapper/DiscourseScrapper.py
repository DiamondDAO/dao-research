    ...
    def sort_users(self, users):
        ...
        for user in users:
            if '.eth' in str(user['user']['name']) or '.eth' in str(user['user']['username']):
                ethUsers.append(user)
            elif '0x' in str(user['user']['name']) or '0x' in str(user['user']['username']):
                ethUsers.append(user)
            else:
                nonEthUsers.append(user)

        with open(os.path.join(self.outpath, "eth_users.json"), 'w') as f:
            f.write(json.dumps(ethUsers))

        with open(os.path.join(self.outpath, "non_eth_users.json"), 'w') as f:
            f.write(json.dumps(nonEthUsers))
        return ethUsers, nonEthUsers
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s','--slug', metavar='category_slug', type=str, 
                        help='IF restricting to one category only, the slug of that category. (requries -i to be set as well)')
    parser.add_argument('-i', '--id', metavar='category_id', type=str, 
